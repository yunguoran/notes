...
- 非替换元素（Non-replaced elements）：没有被外部资源替换的、HTML 自己的内容元素。
    - `<span>`。
    - `<a>`。
    - `<strong>`。
    - `<em>`。
    ...
    - `<div>`。
    - `<p>`。
- 替换元素（Replaced elements）：内容是由外部资源替换进来的，比如：图片或视频文件。
    - `<img>`。
    - `<video>`。
    - `<embed>`。
    - `<iframe>`。
...
CSS 无法影响替换元素的内部布局。浏览器在默认情况下，会使用图片或视频本身的固有尺寸来显示它们，从而保持其固有的宽高比。这意味着：
...
当 `object-fit` 的值为 `cover` 时，图片被裁剪了，`object-position` 可以决定优先保留哪部分，取值可以是：
...
- `p:first-child`：选择所有元素的第一个子元素，**并且仅当**这个第一个子元素是一个 `<p>` 标签。
...
- `tr:nth-child(1)`：表示选中所有作为其父元素第一个子元素的 `tr` 元素。这里 `tr` 元素选择器和 `:nth-child(1)` 伪类选择器之间没有空格，因此是整个选择器是复合选择器。
...
`:any-link` 是一个功能性的伪类选择器，它专门用于匹配所有具有链接锚点的元素。它相当于同时选中了已访问（`:visited`）和未访问（`:link`）状态的链接。在 HTML 中，一个 `<a>` 标签只有在拥有 `href` 属性时才被视为链接，`:any-link` 的作用就是精准定位这些元素。

- 匹配所有带有 `href` 属性的 `<a>`、`<area>` 或 `<link>` 元素。
- 它将 `:link`（未访问）和 `:visited`（已访问）这两个状态合二为一。
...
选择器之间直接相连不加空格就代表的是 `and` 关系。同时拥有 `.alert` 和 `.stop` 属性才选中可以写成：
...
- `border-width`：边框宽度。
    - 长度值（如：1px、2px）。该属性的值只能使用 `<length>` 类型，不能使用 `<percentage>` 类型。
    - 关键字。
        - `thin`。
        - `medium`。
        - `thick`。
...
```css
/* 没有任何渐变，看起来是一整块纯红色背景。
 *  - 浏览器会自动补成 linear-gradient(to bottom, red, red)。
 *  - to bottom 是默认方向，由上至下。
 */
linear-gradient(red)

/* 左下角是蓝色，右上角是红色，中间平滑过渡。
 *  - 方向遵循上（0deg）、右（90deg）、下（180deg）、左（270deg）的顺序。45deg 是从下左到上右。
 *  - 等价于 linear-gradient(to right top, blue, red)。
 */
linear-gradient(45deg, blue, red)

/* 右下角蓝色，左上角红色，中间平滑过渡。
 *  - to left top 是渐变线指向左上角。
 *  - 等价于 linear-gradient(315deg, blue, red)。
 */
linear-gradient(to left top, blue, red)

/* 上方蓝色，底部红色，中间颜色使用 OKLab 颜色空间进行插值。
 *  - 默认方向，由上至下。
 *  - OKLab 颜色间的数值距离约等于人眼感知的差异距离，亮度变化更自然，不容易出现脏灰色。适合现代 UI，优质视觉的场景。
 */
linear-gradient(in oklab, blue, red)

/* 上方蓝色，底部红色，中间颜色使用 HSL 颜色空间进行插值。
 *  - 默认方向，由上至下。
 *  - 使用 HSL 颜色空间进行插值会遍历色相环，表现出鲜艳彩虹色，适合彩虹渐变，避免灰暗的场景。
 *  - 默认沿色环走最短路径，视觉效果是蓝 -> 紫 -> 红渐变。
 */
linear-gradient(in hsl, blue, red)

/* 和 linear-gradient(in hsl, blue, red) 的区别是沿色环长路径插值，视觉效果呈现蓝 -> 绿 -> 黄 -> 橙 -> 红的颜色变化。 */
linear-gradient(in hsl longer hue, blue, red)

/* 底部蓝色，40% 高度处纯绿色，再往上逐渐变成红色。
 *  - 0deg，方向由下至上。
 *  - 颜色停靠点：蓝色在 0% 处，绿色在 40% 处，红色在 100% 处。
 */
linear-gradient(0deg, blue, green 40%, red)

/* 前 10% 快速从红色过渡到中间色，后 90% 慢慢过渡到蓝色。
 *  - 1turn 等于 360°，.25turn 是 90°，方向从左至右。
 *  - 10% 是 color hint，含义是渐变的中点颜色在 10% 的位置出现。此处改变的是渐变的节奏而非颜色的位置。
 */
linear-gradient(.25turn, red, 10%, blue)

/* 45° 对角线，一半红，一半蓝，中间是硬切线，没有过渡模糊。
 *  - 45deg，方向从左下到右上。
 *  - red 0 50% 和 blue 50% 100% 是多位置颜色停靠点。
 *      - red 0 50% 表示红色从渐变线的 0% 处开始，到渐变线的 50% 处结束，即在渐变线的前 50% 完全填充红色。
 *      - blue 50% 100% 表示蓝色从渐变线的 5% 处开始，到渐变线的 100% 处结束，即在渐变线的后 50% 完全填充蓝色。
 */
...
使用 Windows 快捷键 Ctrl + Shift + I 或 F12 打开，也可以直接右键点击网页上的元素，在上下文菜单中选择 *Inspect*，这样会直接高亮该元素的代码。

#### 元素（Elements）

双击一个元素或右键单击该元素，在上下文菜单中选择 *Edit as HTML*。可以直接修改该元素，但无法保存。

### View page source 和 DOM 的区别

| 特性 | **View Page Source（查看页面源代码）** | **DOM（开发者工具中看到的）** |
| :--- | :--- | :--- |
| **本质** | **静态文本文件** | **内存中的动态对象树** |
| **内容来源** | 直接从服务器接收的原始 HTTP 响应体 | 浏览器解析源代码后，经过 JavaScript 修改的当前状态 |
| **获取方式** | 右键 → "查看页面源代码"，或 `Ctrl+U` | 右键 → "检查" 或 `F12`，在 Elements 面板中查看 |
| **是否会变化** | **不会**。页面加载完成后就固定不变 | **会实时变化**。随着 JS 执行和用户交互而动态更新 |
| **包含 JS 修改吗？** | **不包含**。只显示最初的 HTML | **包含**。显示所有 JS 修改后的最终结果 |
| **包含动态内容吗？** | **不包含**（如 AJAX 加载的数据） | **包含** |
| **包含隐藏元素吗？** | 包含（即使 CSS 隐藏了，源代码里也有） | 包含，但开发者工具会做视觉提示（如变灰） |
| **格式** | 纯文本，可能没有缩进或格式混乱 | 格式良好的可交互树，可展开/折叠节点 |

### 解决特异性问题

如发现 CSS 样式没有像你期望的那样运行，可以直接右键点击网页上的元素，选择 *Inspect*，此时该元素的所有 CSS 属性以及属性值会按优先级从高到低显示，未被应用的低优先级的属性值将会被加上删除线。
